...
def RetailRocket():
    ...
    df = df[df['event'] == 'view']  # Filter only view events
    df = df[['timestamp', 'visitorid', 'itemid']]
    # make all items integer
    datas = list(zip(df['visitorid'], df['itemid'], df['timestamp']))
...
if __name__ == "__main__":
    dataname = sys.argv[1]
    available_datasets = ['Beauty', 'Sports_and_Outdoors', 'Toys_and_Games', 'LastFM', 'ML-1M', 'Yelp', "RetailRocket"] # TODO: Yelp
    if dataname == 'all':
