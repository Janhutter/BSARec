...
class FrequencyLayer(nn.Module):
    ...
    def forward(self, input_tensor, save=False):
        ...
        if save:
            iteration = 0
            file_path = os.path.join(self.args.save_path, self.args.data_name, f'{self.args.run_name}_filter_{iteration}.npy')
            ...
            while os.path.exists(file_path):
                iteration += 1
                file_path = os.path.join(self.args.save_path, self.args.data_name, f'{self.args.run_name}_filter_{iteration}.npy')
            np.save(file_path, sequence_emb_fft.cpu().detach().numpy())
