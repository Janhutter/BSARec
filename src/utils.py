...
def parse_args():
    ...
    parser.add_argument("--padding", default="zero", type=str, help="padding type, zero, cyclic, reflect, mirror")
    ...
    # for wandb only
    parser.add_argument("--project", default='BSARec', type=str, help="project name for wandb")
    parser.add_argument("--run_name", default='BSARec', type=str, help="run name for wandb")
    parser.add_argument("--wandb_log_freq_model", type=int, default=10)
    parser.add_argument("--wandb_log_model", action="store_true")

    # saving all model logits
    parser.add_argument("--save", action="store_true", help="save all model logits")
    parser.add_argument("--save_path", default="output/", type=str, help="path to save all model logits")

    parser.add_argument("--spectral", action="store_true", help="output attention")

    args, _ = parser.parse_known_args()

    if 'bsarec' in args.model_type.lower() or 'fourier' in args.model_type.lower():
        parser.add_argument("--c", default=3, type=int)
        parser.add_argument("--alpha", default=0.9, type=float)
        parser.add_argument("--no_padding", action="store_true", help="remove padding after fourier transform in bsarec")
        parser.add_argument("--flip_zero_padding", action="store_true", help="flip zero padding in bsarec")
        if args.model_type.lower() == 'bsarec_wavelet':
            parser.add_argument("--filter_length", default=2, type=int, help="length of wavelet filter")
            parser.add_argument("--wavelet", default='db1', type=str, help="wavelet type")
            parser.add_argument("--wavelet_mode", default='zero', type=str, help="wavelet mode")
    ...
    elif args.model_type.lower() == 'fourierrec':
        parser.add_argument("--c", default=3, type=int)
        parser.add_argument("--alpha", default=0.9, type=float)

    elif args.model_type.lower() == "fourierrec":
        parser.add_argument("--c", default=3, type=int)
    elif args.model_type.lower() == "bsarec2":
        parser.add_argument("--c", default=3, type=int)
        parser.add_argument("--alpha", default=0.9, type=float)
        parser.add_argument("--filter_length", default=2, type=int, help="length of wavelet filter")
    ...
    elif args.model_type.lower() == "ficlrec":
        parser.add_argument("--p", default=5, type=int)
        parser.add_argument("--cl_mode",type=str,default='hl',help="contrastive mode")
...
def write_output(rating_pred, answer_list, file_path):
    """Write the prediction results to a file. Seperated by tab."""
    rating_pred = torch.from_numpy(np.array(rating_pred))
    values, top_indices = torch.topk(rating_pred, k=100, dim=1)
    ...
    output_data = {
        ...
        "answers": np.array(answer_list).tolist()
    }
