...
def Amazon(dataset_name, rating_score):
    ...
    datas = []
    # older Amazon
    data_flie = '/data/reviews_' + dataset_name + '.json.gz'
    # latest Amazon
...
def Yelp(date_min, date_max, rating_score):
    datas = []
    data_flie = 'data/process/raw/yelp_academic_dataset_review.json'
...
def Yelp_meta(datamaps):
    meta_infos = {}
    meta_file = 'data/process/raw/yelp_academic_dataset_business.json'
...
def main(data_name, data_type='Amazon'):
    ...
    # dump full data_maps
    maps_pkl  = f'data/{data_name}_data_maps.pkl'
    maps_json = f'data/{data_name}_data_maps.json'
    with open(maps_pkl, 'wb') as f:
        pickle.dump(data_maps, f)
    with open(maps_json, 'w') as f:
        json.dump(data_maps, f)
    print(f"Saved mapping files: {maps_pkl}, {maps_json}")

    # ── NEW ──
    # for Yelp, also write a simple raw→numeric mapping for business IDs
    if data_type == 'Yelp':
        biz2id_file = f'data/{data_name}_business2id.json'
        with open(biz2id_file, 'w') as f:
            json.dump(data_maps['item2id'], f)
        print(f"Saved raw business→ID mapping: {biz2id_file}")
